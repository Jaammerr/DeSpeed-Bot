        ...
        try:
            ...
            for line in lines:
                try:
                    ...
                    if mode == "default_accounts":
                        email = parts[0].replace(" ", "")
                        yield Account(email=email)

                    elif mode == "register_accounts":
                        if len(parts) == 2:
                            email = parts[0].replace(" ", "")
                            ...
                            email = parts[0].replace(" ", "")
                            password = parts[1].replace(" ", "")
                            account_password = parts[2].replace(" ", "")
                            yield Account(email=email, password=password, account_password=account_password)

                    elif mode == "verify_accounts":
                        if len(parts) >= 3:
                            email = parts[0].replace(" ", "")
                            password = parts[1].replace(" ", "")
                            account_password = parts[2].replace(" ", "")
                    ...
                    elif mode == "login_accounts":
                        if len(parts) == 2:
                            email = parts[0].replace(" ", "")
                            account_password = parts[1].replace(" ", "")
                            yield Account(email=email, account_password=account_password)
    ...
    def _parse_referral_codes(self) -> List[str]:
        try:
            ...
            for code in referral_codes:
                ...
                elif code.startswith("https://app.despeed.net/register?ref="):
                    code = code.split("ref=")[1]

                fixed_referral_codes.append(code)

            return fixed_referral_codes
    ...
    def load(self) -> Config:
        try:
            ...
            if not referral_codes:
                raise ConfigurationError("No referral codes found in referral_codes.txt | Registration without referral codes is prohibited by the DeSpeed itself")
            ...
            if not any([
                ...
                raise ConfigurationError("No accounts found in files: login_accounts.txt, farm_accounts.txt, register_accounts.txt, export_stats_accounts.txt | Please add accounts to the files")
