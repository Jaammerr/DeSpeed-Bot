...
class LinkExtractor:
    ...
    def __init__(
        ...
        self.link_patterns = [
            r"https://app\.despeed\.net/verify-email/([a-f0-9]+)",
            r"https://deref-gmx\.net/mail/client/[^/]+/dereferrer/\?redirectUrl=https%3A%2F%2Fapp\.despeed\.net%2Fverify-email%2F([a-f0-9]+)",
            r"https?://[^\s]+app\.despeed\.net/verify-email/([a-f0-9]+)",
        ]

    async def extract_link(self, proxy: Optional[Proxy] = None, check_msg_age: bool = True) -> OperationResult:
        logger.info(f"Account: {self.email} | Checking email for link...")
        return await self.search_with_retries(proxy, check_msg_age)

    def _collect_messages(self, mailbox: MailBox):
        messages = []

        for msg in mailbox.fetch(reverse=True, criteria=AND(from_="noreply@despeed.net"), limit=10, mark_seen=True):
        ...
        for msg in mailbox.fetch(reverse=True, limit=10, mark_seen=True):
            if msg.from_.startswith("noreply_at_despeed_net") or msg.from_ == "noreply@despeed.net":
                if self.redirect_email and self.redirect_email != msg.to[0]:
                    continue
    ...
    def _process_latest_message(self, messages, check_msg_age: bool = True):
        if not messages:
            return None
    ...
    async def _search_in_all_folders(self, proxy: Optional[Proxy], check_msg_age: bool = True) -> Optional[str]:
        def search_in():
            all_messages = []
            with MailBoxClient(host=self.imap_server, proxy=proxy, timeout=30).login(self.email, self.password) as mailbox:
                ...
                return self._process_latest_message(all_messages, check_msg_age) if all_messages else None

        return await asyncio.to_thread(search_in)

    async def search_with_retries(self, proxy: Optional[Proxy] = None, check_msg_age: bool = True) -> OperationResult:
        for attempt in range(self.max_attempts):
            link = await self._search_in_all_folders(proxy, check_msg_age)
            if link:
                return {
                    "status": True,
                    "identifier": self.email,
